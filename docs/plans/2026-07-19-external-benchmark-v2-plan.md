...
正式抽樣前先建立至少 1,800 筆 input-only candidate pool，並遵守：
...
- 凍結 candidate pool hash 後，以固定 seed `20260719` 在 domain × risk strata 內抽樣；
  importer 必須能從相同 pool、seed 重建完全相同的 600 IDs。
- 抽樣後不得因任何 converter 表現替換案例；只允許因授權、重複、資料毀損或 expected
  無法裁決而排除，且 replacement 必須由同 stratum 的預先排序候補補入並留下 ledger。

正式固定 600 筆前執行 paired-proportion power analysis。以 80% power、雙尾
`alpha = 0.05` 為最低標準，預先假設／敏感度分析 competitor discordant rate，要求
可偵測差異（MDE）不高於 2 個百分點；600 筆不足時，在第一次 benchmark 前擴充至
1,200 筆。報告必須說明 CI 是此 challenge sample 的不確定性，不能推論所有真實流量。
...
只有 final-decision artifact 明確涵蓋的案例才算 human decision；最終 600/600 或
1,200/1,200 都必須可稽核。
...
- [ ] 新增 `LICENSES.md`、attribution 與 output-license validation。
...
- [ ] 實作 sample-size/power analysis 與 preregistration schema。
...
- [ ] 建立至少 1,800 筆 candidate pool，完成來源、授權、exact/near dedupe audit。
- [ ] 凍結 pool hash，以 seed `20260719` 依固定 domain/risk quotas 抽樣。
- [ ] 完成 power analysis；MDE > 2pp 時在首次執行前擴充至 1,200 筆。
- [ ] 執行 Codex first pass。
- [ ] 執行 Gemini independent advisory。
- [ ] 產生差異與 maintainer review packet。
- [ ] Maintainer confirmation 後寫入 private expected。
- [ ] 驗證 final decisions 明確涵蓋 600/600 或 1,200/1,200 cases。
- [ ] 封存 inputs、expected、protocol 與 preregistration hash，再執行 one-shot benchmark。

完成條件：最終抽樣 600/600（若 power analysis 要求擴充則 1,200/1,200）human
decisions 完成，來源標示誠實，第一次執行前無 converter output 接觸 expected。
...
- [ ] blind-v2 至少 600 筆（若 power analysis 要求則 1,200 筆）完成 maintainer human
  decision。
