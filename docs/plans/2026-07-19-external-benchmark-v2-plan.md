...
- [x] Maintainer 於 2026-07-24 確認 batch 014 synthesis，建立 human decision
  artifact；100 筆全數通過 exact/reference 與 character 5-gram Jaccard 0.85
  promotion gate 並寫入 pool。Pool 現為 1,181 筆，距最低 5,880 筆仍差 4,699。
- [x] 建立 project-original formal/entity/identifier guard input-only pilot 100 筆，
  補強 `formal_news` 與 `over_conversion_guard` 缺口；來源明確標示 Codex 起草、
  synthetic coverage 與未使用 converter output／expected，raw 與 normalized
  snapshot 均以 SHA-256 固定。
- [x] 建立 classification batch 015，完成 Codex first pass、Gemini via
  Antigravity CLI (`gemini-3.1-pro-high`) independent advisory 與 Codex synthesis：
  73 筆四欄完全一致、27 筆 domain/risk 差異；綜合建議 100 筆皆 eligible，
  selection basis 為 agreement 73、Codex 12、Gemini 15；第二輪保守複核避免將
  臺灣可接受的「資料搜集／發布修訂版」誤列為 candidate gap，並將無明確法律語境的
  `Exhibit B-4` 案例維持 `formal_news`。Gemini 100/100 ID
  覆蓋、零工具呼叫、零 API 錯誤。
- [x] Maintainer 於 2026-07-24 確認 batch 015 synthesis，建立 human decision
  artifact；100 筆全數通過 exact/reference 與 character 5-gram Jaccard 0.85
  promotion gate 並寫入 pool。Pool 現為 1,281 筆，距最低 5,880 筆仍差 4,599。
- [x] 稽核 CISA `Cyber Hygiene` 單頁簡體中文 PDF：固定 CISA author metadata、
  原始 checksum、頁數、標題與正文 anchors；保守抽取 24 筆 input-only 完整句，
  排除標題、頁面元件、標誌、聯絡資訊、外部連結與第三方內容。
- [x] 建立 classification batch 016，完成 Codex first pass、Gemini via
  Antigravity CLI (`gemini-3.1-pro-high`) independent advisory 與 Codex
  synthesis：1 筆四欄完全一致、23 筆差異；綜合建議 21 筆納入、3 筆因 malformed
  或 context-dependent 排除。Gemini 24/24 ID 覆蓋、零工具呼叫、零 API 錯誤。
  此來源實際補入 19 筆 candidate gap 與 2 筆 baseline，沒有 over-conversion guard。
- [x] Maintainer 於 2026-07-24 確認 batch 016 synthesis，建立 human decision
  artifact；21 筆 eligible 全數通過 exact/reference 與 character 5-gram
  Jaccard 0.85 promotion gate 並寫入 pool，3 筆 excluded 維持排除。Pool 現為
  1,302 筆，距最低 5,880 筆仍差 4,578。
- [x] 稽核 CISA `Personal Security Considerations Action Guide` 八頁簡體中文
  PDF：固定 CISA author/title metadata、原始 checksum、頁數、修訂日期與正文
  anchors；保守抽取 134 筆以標點終止的 input-only 句子候選，排除頁面元件、
  聯絡資訊、外部連結、references 與第三方引用內容；條列片段再由 input-only
  classification 以 fail-closed 原則排除。
- [x] 建立 classification batch 017，完成 Codex first pass、Gemini via
  Antigravity CLI (`gemini-3.1-pro-high`) independent advisory 與 Codex
  synthesis：55 筆四欄完全一致、45 筆差異；Codex 複核後綜合建議 90 筆納入、
  10 筆因 fragment 或 context-dependent 排除，selection basis 為 agreement 55、
...
- [x] 2026-07-27 建立 Kubernetes 第二輪 batch 028：明確排除 batch 027 後，
  ...
  全部 eligible。Maintainer 已確認 Codex synthesis 的 80/10 fail-closed 建議；
  80 筆 eligible 全數通過 promotion，pool `2,326 → 2,406`，距最低 5,880 筆
  尚差 3,474。`permissive_license` 832/2,406（34.58%）、`project_original`
  780/2,406（32.42%）、`public_domain` 794/2,406（33.00%），皆低於 35%。
- [x] 2026-07-27 建立 public-domain 平衡 batch 029：從 Census、CISA、OSHA 與
  四個 Ready.gov 主題各固定抽取 10 筆，明確排除使用相同來源的舊 packets，並
  驗證與 batches 001-028 的 ID 重疊為零。Codex first pass 建議 64/70 eligible；
  Gemini CLI 0.52.0 (`gemini-2.5-pro`) 以 70/70 IDs、零工具呼叫、零 API 錯誤
  完成獨立審查，建議 67/70 eligible。Codex 複核後採 Gemini 納入一筆語意完整的
  短句，最終 synthesis 建議 65 筆納入、5 筆排除。Maintainer 已確認最終
  synthesis 的 65 筆納入、5 筆排除。65 筆全數通過
  promotion，pool `2,406 → 2,471`，距最低 5,880 筆尚差 3,409；
  `public_domain` 859/2,471（34.76%）、`permissive_license` 832/2,471（33.67%）、
  `project_original` 780/2,471（31.57%），皆低於 35%。
- [x] 2026-07-27 建立來源類別平衡 batch 030：從 AOSP、Chromium、Kubernetes、
  MASSIVE、Census 與 Ready.gov Radiation 各固定抽取 16 筆，排除 batches
  001-029 後確認 96 筆 ID 全部跨批唯一。Codex first pass 建議 88/96 eligible；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）在 Codex 結論隱藏的隔離目錄完成 96/96
  input-only 獨立審查，零工具呼叫、零 API 錯誤，建議 96/96 eligible。Codex
  synthesis 複核後建議 88 筆納入、8 筆排除，maintainer 已確認；88 筆全數
...
- [x] 2026-07-27 建立 project-original LLM domain-balance batch 031：起草 100 筆
  ...
  納入，maintainer 已確認；98 筆通過 promotion，另有 1 筆 exact duplicate 與
  1 筆 near duplicate 由固定 dedupe 排除。Pool `2,559 → 2,657`，距最低 5,880
  筆尚差 3,223；`permissive_license` 891/2,657（33.53%）、`public_domain`
  888/2,657（33.42%）、`project_original` 878/2,657（33.04%）。
- [x] 2026-07-27 建立六來源平衡 batch 032：從 AOSP、Census、Chromium、
  Kubernetes、OSHA 與 Ready.gov Home Fires 各固定抽取 16 筆，排除相關舊
  packets 後確認 96 筆 ID 與 batches 001-031 全部不重疊。Codex first pass
  建議 92/96 eligible；Gemini CLI 0.52.0（`gemini-2.5-pro`）因單次 96 筆輸出
  未通過 ID 完整性驗證，改以來源隔離為六組各 16 筆重跑，最終 96/96 IDs、
  六次工具呼叫、零 API 錯誤，建議 94/96 eligible。Codex synthesis 建議 92 筆
  納入、4 筆因 fragment、malformed translation 或欄位／頁首黏連而排除；48 筆
  四欄完全一致、44 筆採 Codex、4 筆採綜合修正。Maintainer 已確認 synthesis，
  建立 96/96 human decision；92 筆 eligible 全數通過 promotion，4 筆來源品質案例
...
- [x] 2026-07-27 建立三來源平衡 batch 034：MASSIVE、Ready.gov Drought 與
  新增的 project-original LLM／日常社群 input-only 語料各固定抽取 32 筆，與
  batches 001-033 的 ID 重疊為零。Codex first pass 建議 93/96 eligible；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）以三個來源隔離 session 完成 96/96 IDs，
  建議 95/96 eligible。Codex synthesis 建議 92 筆納入、4 筆排除；39 筆四欄
  完全一致、50 筆採 Codex、1 筆採 Gemini、6 筆採綜合修正。Maintainer 已
  確認 synthesis，建立 96/96 human decision；92 筆 eligible 全數通過 promotion，
  4 筆來源品質案例維持排除，沒有新增 dedupe exclusion。Pool `2,840 → 2,932`，
  距最低 5,880 筆尚差 2,948；`permissive_license` 1,015/2,932（34.62%）、
  `public_domain` 1,007/2,932（34.34%）、`project_original` 910/2,932
  （31.04%），皆低於 35%。
- [x] 2026-07-27 建立三來源平衡 batch 035：MASSIVE、Census Newsroom 與
  既有 project-original LLM／日常社群 input-only 語料各固定抽取 32 筆，與
  batches 001-034 的 ID 重疊為零。Codex first pass 建議 90/96 eligible；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）以三個來源隔離 session 完成 96/96 IDs，
  三次皆為單一 API request、零工具呼叫、零 API 錯誤，建議 96/96 eligible。
  Codex synthesis 建議 90 筆納入、6 筆因 malformed translation、未閉合引文
  或 context-dependent fragment 排除；61 筆四欄完全一致、30 筆採 Codex、
  5 筆採 Gemini。Maintainer 已確認 synthesis，建立 96/96 human decision；90 筆
  eligible 全數通過 promotion，6 筆來源品質案例維持排除，沒有新增 dedupe
...
- [x] 2026-07-28 建立三來源平衡 batch 036：MASSIVE、Census Newsroom 與
  ...
  26 筆採 Codex、19 筆採 Gemini、6 筆採欄位級綜合。Maintainer 已確認
  synthesis，建立 96/96 human decision；92 筆 eligible 全數通過 promotion，4 筆
  來源品質案例維持排除，沒有新增 dedupe exclusion。Pool `3,022 → 3,114`，距最低
  5,880 筆尚差 2,766；`permissive_license` 1,073/3,114（34.46%）、
  `public_domain` 1,067/3,114（34.26%）、`project_original` 974/3,114（31.28%），
  皆低於 35%。
- [x] 2026-07-28 建立 project-original IT／LLM／social guard v1 input-only
  pilot 100 筆：40 筆 IT 識別碼與協定保護、30 筆 LLM candidate/baseline、30 筆
  social entity guard。Raw 與 normalized SHA-256 已固定，來源內 100/100 ID 與
...
- [x] 2026-07-28 建立 source-class-balanced batch 037：MASSIVE 32 筆、新增
  ...
  （31.41%），皆低於 35%。
- [x] 2026-07-28 建立 source-class-balanced batch 038：MASSIVE 與既有
  project-original guard source 各 32 筆；CISA Personal Security、Ready.gov Home
  Fires、Landslides、Radiation 各 8 筆，共 32 筆 public-domain。排除 batches
  001-037 後固定抽樣。Codex first pass 建議 90 筆納入、6 筆排除；Gemini CLI
  0.52.0 以六個來源隔離 session 完成 96/96 input-only 獨立審查，零工具呼叫、
  零 API 錯誤，建議 94 筆納入、2 筆排除。Codex synthesis 將「播放一席播客」
  修正為可獨立裁決的專名保護案例，最終建議 91 筆納入、5 筆排除；maintainer
  已確認整批 synthesis。91 筆全數通過 promotion，pool `3,203 → 3,294`，距最低
...
- [x] 2026-07-28 建立 source-class-balanced batch 041：MASSIVE 與
  ...
  逐筆複核後維持 87 筆納入、9 筆排除；maintainer 已確認整批 synthesis。
  87 筆 eligible 中，86 筆通過 promotion；Winter Weather `sentence-014` 與既有
  Floods `sentence-016` 完全重複，依固定規則去重排除。Pool `3,467 → 3,553`，
...
- [x] 2026-07-28 建立 source-class-balanced batch 042：AOSP、FTC 新來源、
  ...
  91 筆納入、5 筆排除（50 agreement、35 Codex、8 Gemini、3 hybrid）。Maintainer
  已確認整批 synthesis，91 筆全數通過 promotion；Pool `3,553 → 3,644`，距最低
...
- [x] 2026-07-28 建立 source-class-balanced batch 043：Chromium、Kubernetes、
  ...
  6 Gemini、1 hybrid）。Maintainer 已確認整批 synthesis，94 筆全數通過
  promotion；Pool `3,644 → 3,738`，距最低 5,880 筆尚差 2,142。
...
- [x] 2026-07-28 建立 batch 045：為降低已達 34.13% 的
  ...
  排除（8 agreement、66 Codex、4 Gemini、18 hybrid）。Maintainer 已確認整批
  synthesis；85 筆全數通過 promotion，沒有新增 dedupe exclusion。Pool
  `3,832 → 3,917`，距最低 5,880 筆尚差 1,963。`permissive_license`
  1,308/3,917（33.39%）、`public_domain` 1,331/3,917（33.98%）、
  `project_original` 1,278/3,917（32.63%），皆低於 35%，且沒有來源超過 10%。
- [x] 2026-07-28 建立 batch 046：由 Kubernetes permissive-license
  剩餘候選與新增 project-original formal/LLM over-conversion guard 各 48 筆組成，
  與 batches 001-045 無 ID 重疊。Codex first pass 建議 88 筆納入、8 筆排除；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）以 12 個隔離 session 完成 96/96
  input-only 獨立審查，零工具呼叫、零 API 錯誤，建議 96 筆全數納入。兩者有
  19 筆四欄完全一致、77 筆差異。Codex 第二輪認定 4 筆帶指涉語的完整句仍可
  獨立裁決，另依內容修正 6 筆 domain，synthesis 建議 92 筆納入、4 筆因 Markdown
  抽取遺失命令參數、主詞或欄位內容而排除（19 agreement、67 Codex、10 hybrid）。
  Maintainer 已確認整批 synthesis，建立 96/96 human decision；92 筆 eligible 中
  91 筆通過 promotion，`formal-035` 因與既有案例完全重複而排除。Pool
  `3,917 → 4,008`，距最低 5,880 筆尚差 1,872。`permissive_license`
  1,352/4,008（33.73%）、`public_domain` 1,331/4,008（33.21%）、
  `project_original` 1,325/4,008（33.06%），皆低於 35%，且沒有來源超過 10%。
- [x] 2026-07-28 建立 batch 047：新增 checksum-pinned Ready.gov
  簡體中文「疏散」公有領域來源，共抽出 49 筆 input-only 候選；本批由該來源、
  Kubernetes permissive-license 與 project-original formal/LLM guard 各固定抽取
  32 筆，與 batches 001-046 無 ID 重疊。Codex first pass 建議 92 筆納入、
  4 筆排除；Gemini CLI 0.52.0（`gemini-2.5-pro`）在空白工作目錄以 12 個
  隔離 session 完成 96/96 獨立審查，零工具呼叫、零 API 錯誤，建議 96 筆
  全數納入。兩者有 29 筆四欄完全一致、67 筆差異。Codex 第二輪修正 21 筆
  risk/domain 標記，synthesis 建議 92 筆納入、4 筆因主詞遺失、括號截斷、
  語法殘缺或安全指令疑似反向錯譯而排除（23 agreement、52 Codex、21 hybrid）。
  Maintainer 已確認整批 synthesis，建立 96/96 human decision；92 筆 eligible
  全數通過 promotion，沒有新增 dedupe exclusion。Pool `4,008 → 4,100`，距最低
  5,880 筆尚差 1,780。`permissive_license` 1,382/4,100（33.71%）、
  `public_domain` 1,361/4,100（33.20%）、`project_original` 1,357/4,100
  （33.10%），皆低於 35%，且沒有來源超過 10%。
- [x] 2026-07-28 建立 batch 048：新增 checksum-pinned Ready.gov
  簡體中文「網路安全」公有領域來源 55 筆，以及 project-original formal/LLM
  context guard 100 筆；連同 Kubernetes permissive-license 各固定抽取 32 筆，
  與 batches 001-047 無 ID 重疊。Codex first pass 建議 87 筆納入、9 筆排除；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）在空白工作目錄以 12 個隔離 session
  完成 96/96 input-only 獨立審查，零工具呼叫、零 API 錯誤，建議 91 筆納入、
  5 筆排除。兩者有 35 筆四欄完全一致、61 筆差異；Codex 第二輪修正 21 筆，
  synthesis 建議 86 筆納入、10 筆因抽取缺漏、錯譯、清單殘片、代名詞依賴或
  第三方資源文案而排除（35 agreement、40 Codex、21 hybrid）。Maintainer 於
  2026-07-29 確認整批 synthesis，建立 96/96 human decision；86 筆 eligible
  全數通過 promotion，沒有新增 dedupe exclusion。Pool `4,100 → 4,186`，距最低
  5,880 筆尚差 1,694。`permissive_license` 1,411/4,186（33.71%）、
  `project_original` 1,389/4,186（33.18%）、`public_domain` 1,386/4,186
  （33.11%），皆低於 35%，且沒有來源超過 10%。
- [x] 2026-07-29 建立 batch 049：由 Kubernetes permissive-license、
  Ready.gov 網路安全／疏散／兒童龍捲風公有領域剩餘候選，以及 project-original
  formal/LLM context guard 各固定抽取 32 筆，與 batches 001-048 無 ID 重疊。
  Codex first pass 建議 88 筆納入、8 筆排除；Gemini CLI 0.52.0
  （`gemini-2.5-pro`）在空白工作目錄以 12 個隔離 session 完成 96/96 input-only
  獨立審查，零工具呼叫、零 API 錯誤，建議 95 筆納入、1 筆排除。兩者有 34 筆
  四欄完全一致、62 筆差異；Codex 第二輪依 Gemini 意見修正 15 筆 risk，
  synthesis 建議 88 筆納入、8 筆因表格殘片、括號截斷、詞序重複、句界缺失、
  清單殘片、錯譯、指涉不明或第三方資源文案而排除（34 agreement、47 Codex、
  15 hybrid）。Maintainer 於 2026-07-29 確認整批 synthesis，建立 96/96 human
  decision；88 筆 eligible 全數通過 promotion，沒有新增 dedupe exclusion。Pool
...
- [x] 2026-07-29 建立 batch 050：納入 Kubernetes 34 筆、Ready.gov
  ...
  77 筆納入、3 筆排除；Gemini CLI 0.52.0（`gemini-2.5-pro`）在空白工作目錄
  以 10 個隔離 session 完成 80/80 input-only 獨立審查，納入報告的 sessions
...
- [x] 2026-07-29 建立 batch 051：由 Kubernetes permissive-license、新增的
  FEMA《Are You Ready?》公有領域簡體指南，以及新增的 project-original
  formal/LLM evidence guard 各固定抽取 32 筆，共 96 筆，與 batches 001-050
  無 ID 重疊。Codex first pass 建議 90 筆納入、6 筆排除；Gemini CLI
  （`gemini-2.5-pro`）以 6 個隔離 session 完成 96/96 input-only 獨立審查，
  零工具呼叫、零 API 錯誤，建議 91 筆納入、5 筆排除。兩者有 29 筆四欄完全
  一致、67 筆差異；Codex synthesis 建議 90 筆納入、6 筆因表格欄位合併、
  括號截斷、標題與項目混接、清單重複、清單殘片或標題與正文混接而排除
  （29 agreement、67 codex_synthesis）。Maintainer 於 2026-07-29 確認整批
  synthesis，建立 96/96 human decision；90 筆 eligible 全數通過 promotion，
  沒有新增 dedupe exclusion。Pool `4,350 → 4,440`，距最低 5,880 筆尚差
  1,440。`permissive_license` 1,499/4,440（33.76%）、`project_original`
  1,487/4,440（33.49%）、`public_domain` 1,454/4,440（32.75%），皆低於 35%，
  且沒有來源超過 10%。
- [x] 2026-07-29 建立 batch 052：由 Kubernetes permissive-license、FEMA
  《Are You Ready?》公有領域簡體指南，以及 project-original formal/LLM
  evidence guard 各固定抽取 32 筆，共 96 筆，與 batches 001-051 無 ID 重疊。
  Codex first pass 建議 81 筆納入、15 筆排除；Gemini CLI 0.52.0
  （`gemini-2.5-pro`）以 6 個隔離 session 完成 96/96 input-only 獨立審查，
  零工具呼叫、零 API 錯誤，建議 89 筆納入、7 筆排除。兩者有 51 筆四欄完全
  一致、45 筆差異；Codex synthesis 建議 81 筆納入、15 筆因句子殘缺、重複
  抽取、標題混接、指涉缺失、單位遺失或來源語意不安全而排除（51 agreement、
  45 codex_synthesis）。Maintainer 於 2026-07-29 確認整批 synthesis，建立
  96/96 human decision；81 筆 eligible 全數通過 promotion，沒有新增 dedupe
  exclusion。Pool `4,440 → 4,521`，距最低 5,880 筆尚差 1,359。
  `permissive_license` 1,526/4,521（33.75%）、`project_original`
  1,519/4,521（33.60%）、`public_domain` 1,476/4,521（32.65%），皆低於 35%，
  且沒有來源超過 10%。
- [x] 2026-07-30 完成 batch 053：由 Kubernetes permissive-license、FEMA
  《Are You Ready?》公有領域簡體指南，以及 project-original formal/LLM
  evidence guard 各固定抽取 32 筆，共 96 筆，與 batches 001-052 無 ID 重疊。
  Codex first pass 建議 87 筆納入、9 筆排除；Gemini CLI 0.52.0
  （`gemini-2.5-pro`）以 6 個隔離 session 完成 96/96 input-only 獨立審查，
  零工具呼叫、零 API 錯誤，建議 92 筆納入、4 筆排除。兩者有 40 筆四欄完全
  一致、56 筆差異；Codex synthesis 採納 Gemini 對 `F E M A` 異常空格的排除
  意見，最終建議 86 筆納入、10 筆因句子殘缺、重複抽取、標題混接、句界缺失
  或縮寫格式損壞而排除（40 agreement、56 codex_synthesis）。Maintainer 於
  2026-07-30 確認整批 synthesis，建立 96/96 human decision；86 筆 eligible
  全數通過 promotion，沒有新增 dedupe exclusion。Pool `4,521 → 4,607`，距最低
...
- [x] 2026-07-30 完成 batch 055：為稀釋 Kubernetes 占比，改由 AOSP
  permissive-license、FEMA《Are You Ready?》公有領域簡體指南，以及
  project-original UI/social baseline guard 各固定抽取 32 筆，共 96 筆，與
  batches 001-054 無 ID 重疊。Codex first pass 建議 89 筆納入、7 筆排除；
  Gemini CLI 0.52.0（`gemini-2.5-pro`）以 6 個隔離 session 完成 96/96
  input-only 獨立審查，零工具呼叫、零 API 錯誤，建議 94 筆納入、2 筆排除。
  兩者有 61 筆四欄完全一致、35 筆差異；Codex synthesis 恢復 1 筆不影響轉換
  判定的代名詞案例，最終建議 90 筆納入、6 筆因來源誤譯、句子缺漏、清單殘片
  或重複抽取而排除（61 agreement、35 codex_synthesis）。Maintainer 於
  2026-07-30 確認整批 synthesis，建立 96/96 human decision；90 筆 eligible
  中 89 筆通過 promotion，1 筆因與既有 project-original 案例完全重複而排除。
  Pool `4,698 → 4,787`，距最低 5,880 筆尚差 1,093。`permissive_license`
  1,618/4,787（33.80%）、`project_original` 1,614/4,787（33.72%）、
  `public_domain` 1,555/4,787（32.48%），皆低於 35%。Kubernetes 目前
  507/4,787（10.59%），仍超過最終單一來源 10% freeze gate；後續批次繼續
  暫停抽取 Kubernetes。
- [x] 2026-07-30 完成 batch 056：延續 AOSP permissive-license、FEMA
  《Are You Ready?》公有領域簡體指南，以及 project-original UI/social
  baseline guard 各固定抽取 32 筆，共 96 筆，與 batches 001-055 無 ID 重疊。
  Codex first pass 建議 90 筆納入、6 筆排除；Gemini CLI 0.52.0
  （`gemini-2.5-pro`）以 6 個隔離 session 完成 96/96 input-only 獨立審查，
  零工具呼叫、零 API 錯誤，建議 94 筆納入、2 筆排除。兩者有 63 筆四欄完全
  一致、33 筆差異；Codex synthesis 恢復 1 筆可獨立判定的避難所術語案例，
  最終建議 91 筆納入、5 筆因符號切斷、清單殘片、標題混接、嚴重抽取損壞或
  重複抽取而排除（63 agreement、33 codex_synthesis）。Maintainer 於
  2026-07-30 確認整批 synthesis，建立 96/96 human decision；91 筆 eligible
  中 90 筆通過 promotion，AOSP 的「重新启动」因與既有 Chromium 案例完全
  重複而排除。Pool `4,787 → 4,877`，距最低 5,880 筆尚差 1,003。
  `permissive_license` 1,649/4,877（33.81%）、`project_original`
  1,646/4,877（33.75%）、`public_domain` 1,582/4,877（32.44%），皆低於 35%。
  Kubernetes 目前 507/4,877（10.40%），仍超過最終單一來源 10% freeze gate；
  後續批次繼續暫停抽取 Kubernetes，且需新增 project-original 來源。
- [x] 2026-07-30 完成 batch 057：由 AOSP permissive-license、FEMA
  《Are You Ready?》公有領域簡體指南，以及新增的 project-original LLM/formal
  reasoning guard 各固定抽取 32 筆，共 96 筆，與 batches 001-056 無 ID 重疊。
  Codex first pass 建議 92 筆納入、4 筆排除；Gemini CLI 0.52.0
  （`gemini-2.5-pro`）以 6 個有效隔離 session 完成 96/96 input-only 獨立審查，
  零工具呼叫、零 API 錯誤；另有 1 個嘗試不可用工具的 session 全數捨棄並重跑。
  Gemini 建議 94 筆納入、2 筆排除。兩者有 31 筆四欄完全一致、65 筆差異；
  Codex synthesis 最終建議 93 筆納入、3 筆因重複抽取、不安全的來源譯文或標題
  混接而排除（31 agreement、65 codex_synthesis）。Maintainer 於 2026-07-30
  確認整批 synthesis，建立 96/96 human decision；93 筆 eligible 全數通過
  promotion，沒有新增 dedupe exclusion。Pool `4,877 → 4,970`，距最低 5,880 筆
  尚差 910。`permissive_license` 1,681/4,970（33.82%）、`project_original`
...
- [x] 2026-07-30 完成 batch 058：延續 AOSP permissive-license、FEMA
  ...
  暫停抽取 Kubernetes。
- [x] 2026-07-30 完成 batch 059：延續 AOSP permissive-license、FEMA
  《Are You Ready?》公有領域簡體指南，以及 project-original LLM/formal
  reasoning guard 各固定抽取 32 筆，共 96 筆，與 batches 001-058 無 ID 重疊。
  Codex first pass 建議 90 筆納入、6 筆排除；Agy CLI 1.1.8 以
  `gemini-3.1-pro-high` 和 6 個隔離單輪 conversation 完成 96/96 input-only
  獨立審查，零工具結果回合、零錯誤，建議 92 筆納入、4 筆排除。兩者有
  30 筆四欄完全一致、66 筆差異；Codex synthesis 經 pinned AOSP XML 複核後
  保留 flattened markup 案例排除，另恢復 1 筆完整 checklist item，最終建議
  90 筆納入、6 筆因格式損壞、重複抽取、來源誤譯或標題混接而排除
  （30 agreement、66 codex_synthesis）。Maintainer 於 2026-07-30 確認整批
  synthesis，建立 96/96 human decision；90 筆 eligible 全數通過 promotion，
  沒有新增 dedupe exclusion。Pool `5,062 → 5,152`，距最低 5,880 筆尚差 728。
  `permissive_license` 1,743/5,152（33.83%）、`project_original`
  1,742/5,152（33.81%）、`public_domain` 1,667/5,152（32.36%），皆低於 35%。
  Kubernetes 目前 507/5,152（9.84%），已低於最終單一來源 10% freeze gate；
  目前所有來源與來源類別皆符合比例限制。
- [x] 2026-07-30 完成 batch 060：新增 project-original LLM/formal operations
  guard 100 筆 input-only 語料，固定 raw SHA-256
  `47ddbc434c1a8e0c9a0445f8025ad2340488bb0ed21692df05608bf3d9efbc94` 與
  normalized SHA-256
  `441878092bca068da717a5162b799b8a164cf476fc9d9937be46415dbce83fe9`；再由
  AOSP permissive-license、FEMA《Are You Ready?》公有領域簡體指南與此新來源
  各固定抽取 32 筆，共 96 筆。Codex first pass 建議 88 筆納入、8 筆排除；
  Agy CLI 1.1.8 以 `gemini-3.1-pro-high` 和 6 個隔離單輪 conversation 完成
  96/96 input-only 獨立審查，零工具結果回合、零錯誤，建議 89 筆納入、7 筆
  排除。兩者有 47 筆四欄完全一致、49 筆差異；Codex synthesis 最終建議 88 筆
  納入、8 筆因來源抽取損壞、重複片段或缺少裁決語境而排除（47 agreement、
  49 codex_synthesis）。Maintainer 於 2026-07-30 確認整批 synthesis，建立
  96/96 human decision；88 筆 eligible 中 87 筆通過 promotion，AOSP 的
  「所有语言」因與既有 Chromium case 完全重複而排除。Pool `5,152 → 5,239`，
  距最低 5,880 筆尚差 641。`permissive_license` 1,774/5,239（33.86%）、
  `project_original` 1,774/5,239（33.86%）、`public_domain` 1,691/5,239
  （32.28%），皆低於 35%。Kubernetes 目前 507/5,239（9.68%）；所有來源與
  來源類別皆符合比例限制。
- [x] 2026-07-30 完成 batch 061：延續 AOSP
  permissive-license、FEMA《Are You Ready?》公有領域簡體指南，以及
  project-original LLM/formal operations guard 各固定抽取 32 筆，共 96 筆，
  與 batches 001-060 無 ID 重疊。Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` input-only 獨立審查均建議 93 筆納入、3 筆排除；96 筆
  四欄完全一致，Codex synthesis 全數為 agreement。排除項目為 1 筆損壞抽取
  （「行 • 李袋」）與 2 筆標題、正文混接。Agy 最終採用 6 個隔離單輪
  conversation，零工具結果回合；另完整記錄並捨棄 1 次送出 token 前的 structured
  output schema error，以及 1 個僅回傳 15/16 筆的不完整 conversation。Maintainer
  於 2026-07-30 確認整批 synthesis；同時修正 decision schema 不合理地要求至少
  1 筆 disagreement 的限制，使 96/96 完全一致的批次可合法記錄。93 筆 eligible
  中 92 筆通過 promotion；Ready.gov sentence 158 因與既有 Floods case 完全重複
  而排除。Pool `5,239 → 5,331`，距最低 5,880 筆尚差 549。
...
- [x] 2026-07-30 完成 batch 062：由 AOSP
  permissive-license、FEMA《Are You Ready?》公有領域簡體指南，以及
  project-original LLM/formal operations guard 各固定抽取 32 筆，共 96 筆，
  ...
  （94 agreement、2 codex_synthesis）。Maintainer 於 2026-07-30 確認整批
  synthesis；95 筆 eligible 全數通過 promotion，沒有新增 dedupe exclusion。
  Pool `5,331 → 5,426`，距最低 5,880 筆尚差 454。`permissive_license`
  1,838/5,426（33.87%）、`project_original` 1,838/5,426（33.87%）、
  `public_domain` 1,750/5,426（32.25%），皆低於 35%。Kubernetes 目前
...
- [x] 2026-07-30 完成 batch 063：新增 project-original
  ...
  normalized SHA-256
  `0c2dcb76333806c3efc5129ddf9bb2854bfd3faa56634399add3d9749b345a83`；再由
  AOSP permissive-license、FEMA《Are You Ready?》公有領域簡體指南與此新來源
  各固定抽取 32 筆，共 96 筆。Codex first pass 建議 91 筆納入、5 筆排除；
  Agy CLI 1.1.8 `gemini-3.1-pro-high` 以 6 個隔離單輪 conversation 完成
  96/96 input-only 獨立審查，零工具結果回合、零錯誤，建議 93 筆納入、3 筆
...
- [x] 2026-07-30 完成 batch 064：依前批比例風險暫停 AOSP，改由
  Chromium permissive-license UI 字串、FEMA《Are You Ready?》公有領域簡體
  指南，以及 project-original social/formal context guard 各固定抽取 32 筆，共
  96 筆，與 batches 001-063 無 ID 重疊。Codex first pass 建議 90 筆納入、6 筆
  排除；Agy CLI 1.1.8 `gemini-3.1-pro-high` 以 6 個隔離單輪 conversation 完成
  96/96 input-only 獨立審查，零工具結果回合、零錯誤，建議 88 筆納入、8 筆
  排除。前 4 個 Agy 回應因使用 Markdown code fence、不符合 raw JSON 契約而完整
  捨棄並重跑，所有重試均保留 audit 記錄。兩者有 94 筆四欄完全一致、2 筆差異；
  Codex synthesis 複核後採 Agy 對 2 筆 Ready.gov 損壞抽取／不完整句的排除判斷，
  最終建議 88 筆納入、8 筆排除（94 agreement、2 codex_synthesis）。Maintainer
  於 2026-07-30 確認整批 synthesis；88 筆 eligible 全數通過 promotion，沒有新增
  dedupe exclusion。Pool `5,518 → 5,606`，距最低 5,880 筆尚差 274。
...
- [x] 2026-07-30 完成 batch 065：延續 Chromium permissive-license
  ...
  結果回合、零錯誤。Maintainer 於 2026-07-30 確認整批 synthesis；90 筆
  eligible 中 86 筆通過 promotion，另有 3 筆 Chromium 與 1 筆 Ready.gov 因
  exact/near dedupe 排除。Pool `5,606 → 5,692`，距最低 5,880 筆尚差 188。
  `permissive_license` 1,931/5,692（33.92%）、`project_original` 1,934/5,692
...
- [x] 2026-07-30 完成 batch 066：新增 project-original
  ...
  normalized SHA-256
  `83f3e4965b6cfb8e1f9754e82f854635282b5b307941b8d5fc5918529ff63277`；新來源
  與現有 5,692 筆 pool 無 exact/near duplicate。再由 Chromium、FEMA《Are You
  Ready?》與此新來源各固定抽取 32 筆，共 96 筆。Codex first pass 建議 90 筆
  納入、6 筆排除；Agy CLI 1.1.8 `gemini-3.1-pro-high` 建議 89 筆納入、7 筆
...
- [x] 2026-07-30 完成 batch 067：因 pool 距最低門檻尚差 98 筆，改用
  ...
  結果回合、零錯誤。Maintainer 於 2026-07-30 確認整批 synthesis；114 筆
  eligible 全數通過 promotion，沒有新增 dedupe exclusion。Pool `5,782 → 5,896`，
  超過 5,880 筆最低門檻 16 筆。`permissive_license` 2,003/5,896（33.97%）、
  `project_original` 2,006/5,896（34.02%）、`public_domain` 1,887/5,896
...
- [x] 2026-07-30 完成 Blind-v2 pool freeze 與正式抽樣：將 5,896 筆 input-only
  ...
  仍不得查看 zhtw 或競品輸出。
- [x] 2026-07-30 完成正式 expected annotation batch 001：從 frozen inputs 的固定
  offset 0 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  ...
  54 筆一致、46 筆不同；Codex synthesis 為 54 agreement、28 Codex、10 Agy、
  8 hybrid，其中 11 筆因高風險、語意或臺灣術語交由 maintainer 確認。Maintainer
  於 2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入 100 筆並維持
  `annotating`，SHA-256 為
  `6fa169beed492487a4e773e9edc8c5d460c2796465bf511e3c13d9eb6ae8c4fb`。Public
  annotation progress 為 100/1,960，SHA-256 為
  `7af8d2e4aa3994b506d5c31fe48f2d88d63ac1a14800811bfa171aae50b59df9`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 002：從 frozen inputs
  的固定 offset 100 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  43 筆一致、57 筆不同；Codex synthesis 為 43 agreement、48 Codex、3 Agy、
  6 hybrid，其中 6 筆因語意或臺灣技術術語交由 maintainer 確認。Maintainer 於
  2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入 200 筆並維持
  `annotating`，SHA-256 為
  `6b6ce3e55f960a5ea8e0ccd1af702cfd40f496e6cd2511e9f1081ef1311894d1`。Public
  annotation progress 為 200/1,960，SHA-256 為
  `cf57209a8a6af777b64d8258a8324c10d227957d753272868a3c15f1804039de`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 003：從 frozen inputs
  ...
  Maintainer 於 2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入
  300 筆並維持 `annotating`，SHA-256 為
  `4adfc88322ac113ad04d1966f5b12f0d367c08ca1a94e6fa88eb3ac8c1194fef`。Public
  annotation progress 為 300/1,960，SHA-256 為
  `3a4b7a63f39f216cde21b9be770a0bbb4496ef1896eba0087e841caa001c5085`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 004：從 frozen inputs
  的固定 offset 300 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 首次執行的第三組回傳 ID
  或順序不符 sealed packet，整次執行未採用；重新執行後取得 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  36 筆一致、64 筆不同；Codex synthesis 為 36 agreement、51 Codex、4 Agy、
  9 hybrid，其中 4 筆因來源語意或 Android 產品術語交由 maintainer 確認。
  Maintainer 於 2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入
  400 筆並維持 `annotating`，SHA-256 為
  `956e55038b2571981a2bd81e3d629020d88d2142f81f83bfd61078510fb82d4c`。Public
  annotation progress 為 400/1,960，SHA-256 為
  `bc1f6b5d588159bb704351ddc415c57cf09015c132d3ffef39c7fa7de1018e1e`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 005：從 frozen inputs
  的固定 offset 400 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  51 筆一致、49 筆不同；Codex synthesis 為 51 agreement、43 Codex、1 Agy、
  5 hybrid，其中 5 筆因電信、資安或來源語意交由 maintainer 確認。Maintainer 於
  2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入 500 筆並維持
  `annotating`，SHA-256 為
  `4ebc97865074c8cbdbcc5888f957550ccd7f9deb79385d27d409af030c060870`。Public
  annotation progress 為 500/1,960，SHA-256 為
  `f7f4dc8b2299513e1130394a33e62e2e28849c85a5d65f3da36cacf7ac5c9817`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 006：從 frozen inputs
  的固定 offset 500 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  57 筆一致、43 筆不同；Codex synthesis 為 57 agreement、35 Codex、4 Agy、
  4 hybrid，其中 4 筆因 RAG、隱私比喻、MFA 或 OSHA 來源語意交由 maintainer
  確認。Maintainer 於 2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入
  600 筆並維持 `annotating`，SHA-256 為
  `6ba0ee0d91cea0305b6bde27cef78265138414a0cf576dc37f07688c95fc09ae`。Public
  annotation progress 為 600/1,960，SHA-256 為
  `0668b3cf57d2f92398ca6f5afc7b0ec77fb7e171ccc8b310b0d992da12423328`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 007：從 frozen inputs
  的固定 offset 600 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 前兩次完整執行的第一組回傳
  ID 或順序不符 sealed packet，整次執行均未採用；另一次隔離診斷 conversation 僅確認
  schema 與順序可正常回傳，也未納入 advisory。其後完整執行取得 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  45 筆一致、55 筆不同，另有 19 筆僅 acceptable variants 不同；Codex synthesis 為
  45 agreement、46 Codex、3 Agy、6 hybrid，其中 8 筆因排水設備、壁爐安全、緊急聯絡人、
  highlight、研究預先註冊或農田 field 語意交由 maintainer 確認。Maintainer 於
  2026-07-30 確認完整 100 筆 synthesis；private expected 已寫入 700 筆並維持
  `annotating`，SHA-256 為
  `654feadb3d76fecd697ff5252ccb5d5a23ff0314d2da7e1ede96b22a985c46e1`。Public
  annotation progress 為 700/1,960，SHA-256 為
  `696ae152e90cf7493005e5b02ed21943ac2c048fbc47f3f7985c69343404ad36`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 008：從 frozen inputs
  的固定 offset 700 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  16 筆一致、84 筆不同，另有 10 筆僅 acceptable variants 不同；Codex synthesis 為
  16 agreement、78 Codex、3 Agy、3 hybrid，其中 10 筆因 Census 作業名稱、來源語意
  缺漏或 Kubernetes lifecycle 語意交由 maintainer 確認。Maintainer 於 2026-07-30
  確認完整 100 筆 synthesis；private expected 已寫入 800 筆並維持 `annotating`，
  SHA-256 為
  `ffd2ef30c077ef1d5d4f036429d97841eb38363b6c6b873368aee62845a342bf`。Public
  annotation progress 為 800/1,960，SHA-256 為
  `0a7aee931176c32f4ca904df0208d96ea9496ce369354a5450153b438a5cab41`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 009：從 frozen inputs
  的固定 offset 800 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  ...
  指令格式、安全警示或 Census 語意交由 maintainer 確認。Maintainer 於 2026-07-30
  確認完整 100 筆 synthesis；private expected 已寫入 900 筆並維持 `annotating`，
  SHA-256 為
  `73c4a41f5fc255ef189dab8d20de9bf06acd28cca8396643138b9d29e43e2abe`。Public
  annotation progress 為 900/1,960，SHA-256 為
  `0204d282ae02a1d4ee4f2b8499d2f052377f96293f77cdca4fbae9fe1f91febf`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 010：從 frozen inputs
  的固定 offset 900 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  33 筆一致、67 筆不同，另有 24 筆僅 acceptable variants 不同；Codex synthesis 為
  33 agreement、67 Codex、0 Agy、0 hybrid，其中 13 筆因來源片段、郵件語意、作品名稱、
  高風險安全文字或技術術語交由 maintainer 確認。Maintainer 於 2026-07-30 確認完整
  100 筆 synthesis；private expected 已寫入 1,000 筆並維持 `annotating`，SHA-256 為
  `5fc54d86badb6e8ffe34de00ff9de0a57f8053d51cbd1b95bca8fce5aad4b63b`。Public
  annotation progress 為 1,000/1,960，SHA-256 為
  `65810f2803dcd7d01663053af81f747f0006f0c3b09df4cb4c8530fb26bfb62a`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 011：從 frozen inputs
  的固定 offset 1,000 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  20 筆一致、80 筆不同，另有 15 筆僅 acceptable variants 不同；Codex synthesis 為
  20 agreement、73 Codex、1 Agy、6 hybrid，其中 15 筆因 Kubernetes、OSHA、Census
  專有詞、實體名稱或來源指令語意交由 maintainer 確認。Maintainer 於 2026-07-30
  確認完整 100 筆 synthesis；private expected 已寫入 1,100 筆並維持 `annotating`，
  SHA-256 為
  `e81b2aa17389297a8d2c4cf9cad7ed4497662fe7769ac3133ea162bf7397eb24`。Public
  annotation progress 為 1,100/1,960，SHA-256 為
  `323cc21fedf083cac1078223455b301b6a07adfa22b95dde26e050c06c4f18c1`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 012：從 frozen inputs
  ...
  安全術語或時區語境交由 maintainer 確認。Maintainer 於 2026-07-30 確認完整 100 筆
  synthesis；private expected 已寫入 1,200 筆並維持 `annotating`，SHA-256 為
  `a4ab0ceb1cbacf97870c15386835638b56b981203a976deba33e92e34cab5d50`。Public
  annotation progress 為 1,200/1,960，SHA-256 為
  `107fdfbfb919291824570a8b563cc6a44b3c511586cf060b21963b2d615d5d11`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 013：從 frozen inputs
  的固定 offset 1,200 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  31 筆一致、69 筆不同，另有 17 筆僅 acceptable variants 不同；Codex synthesis 為
  31 agreement、67 Codex、1 Agy、1 hybrid，其中 13 筆因 Android、Kubernetes、反詐騙
  或緊急警報術語交由 maintainer 確認。Maintainer 於 2026-07-30 確認完整 100 筆
  synthesis；private expected 已寫入 1,300 筆並維持 `annotating`，SHA-256 為
  `dbcca71dd2697ce219ae97756042c4baac741b0ab0d077e773534ef1dbd88b1a`。Public
  annotation progress 為 1,300/1,960，SHA-256 為
  `ed1fe240427b8fd0c649f98bf99fec06e78ab808422b3349712af0debdc37dbc`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 014：從 frozen inputs
  的固定 offset 1,300 取 100 筆，Codex first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  38 筆一致、62 筆不同，另有 20 筆僅 acceptable variants 不同；Codex 完整複核並
  修正 2 筆初稿後，synthesis 為 38 agreement、49 Codex、7 Agy、6 hybrid，其中 15 筆
  因資料庫、Census、美國文件與機構、來源指令、緊急應變或 Kubernetes 術語交由
  maintainer 確認。Maintainer 於 2026-07-30 確認完整 100 筆 synthesis；private
  expected 已寫入 1,400 筆並維持 `annotating`，SHA-256 為
  `484e167ec8d346608387e70cdbfdc459dee8043491830728f2d0c4eb86ba567d`。Public
  annotation progress 為 1,400/1,960，SHA-256 為
  `86da787ce55720957ac1f363be7e42e68501470bb9891bcedcae83137c8a96c4`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 015：從 frozen inputs
  的固定 offset 1,400 取 100 筆，Codex CLI `gpt-5.6-sol` first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  單輪 conversation，每組 20 筆，100/100 IDs 與順序驗證通過。雙方 primary expected
  48 筆一致、52 筆不同，另有 29 筆僅 acceptable variants 不同；Codex 完整複核並
  修正 2 筆 synthesis 初稿後，結果為 48 agreement、41 Codex、4 Agy、7 hybrid，
  其中 12 筆因緊急應變、受管制運輸詞彙、身分盜用通報、Kubernetes 語意或美國機構
  正式名稱交由 maintainer 確認。Maintainer 於 2026-07-30 確認完整 100 筆
  synthesis；private expected 已寫入 1,500 筆並維持 `annotating`，SHA-256 為
  `f61a5b8b27c87ae60d90a54d851901b75482b9606c33a97a080c66d0b2b7def6`。Public
  annotation progress 為 1,500/1,960，SHA-256 為
  `6b9001cd7378540f0fafec224db6925fa1ee2075e999026f6872c8905a228e77`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-30 完成正式 expected annotation batch 016：從 frozen inputs
  的固定 offset 1,500 取 100 筆，Codex CLI `gpt-5.6-sol` first pass 與 Agy CLI 1.1.8
  `gemini-3.1-pro-high` independent advisory 均完成。Agy 使用 5 個隔離、sandboxed、
  ...
  數值用語交由 maintainer 確認。Maintainer 於 2026-07-30 確認完整 100 筆
  synthesis；private expected 已寫入 1,600 筆並維持 `annotating`，SHA-256 為
  `f5731ad880ea7cbc4357e9b26e7e6b4f25a81b9874831de099e1fceacecfa752`。Public
  annotation progress 為 1,600/1,960，SHA-256 為
  `1af490d191a657f055a0c2635483a1ce22d6e5fd4be94f39e748b84d3ab9cc14`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-31 完成正式 expected annotation batch 017：從 frozen inputs
  的固定 offset 1,600 取 100 筆。Codex CLI `gpt-5.6-sol` first pass 的無輸出任務
  全數丟棄，accepted output 由 8 個 input-only 群組（25/5/5/5/5/5/25/25）合併，
  100/100 IDs 與順序通過 canonical validation；Agy CLI 1.1.8
  `gemini-3.1-pro-high` 使用 5 個隔離、sandboxed、單輪 conversation，每組 20 筆。
  雙方 primary expected 58 筆一致、42 筆不同，另有 39 筆僅 acceptable variants
  不同；Codex 以 3 個 input-only 群組（20/20/2）完成 synthesis 並逐筆複核，修正
  1 筆初稿後結果為 58 agreement、29 Codex、8 Agy、5 hybrid，其中 17 筆因
  Kubernetes 語意、Android UI 或 11 筆 high-stakes 應變／保險文字交由 maintainer
  確認。Maintainer 於 2026-07-31 確認完整 100 筆 synthesis；private expected
  已寫入 1,700 筆並維持 `annotating`，SHA-256 為
  `d4b5aae26873faff2e916378046ab6eb5c2ec83de54cad35ac133673f39e24b6`。Public
  annotation progress 為 1,700/1,960，SHA-256 為
  `487e04c0e68b0338fe46ba627bf9631069e0a0fe9e84e533efb3b8799327fcec`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-31 完成正式 expected annotation batch 018：從 frozen inputs 的
  固定 offset 1,700 取 100 筆。Codex first pass 的
  accepted output 由 12 個 input-only 群組（5/5/5/5/5/25/5/5/5/5/5/25）合併，
  5 個無輸出任務已丟棄；100/100 IDs 與順序通過 canonical validation。Agy CLI
  1.1.8 `gemini-3.1-pro-high` 使用 5 個隔離、sandboxed、單輪 conversation，
  每組 20 筆。雙方 primary expected 74 筆一致、26 筆不同，另有 56 筆僅
  acceptable variants 不同；Codex 以 2 個 input-only 群組（20/6）完成 synthesis
  並逐筆複核，結果為 74 agreement、19 Codex、7 Agy、0 hybrid。10 筆差異案例與
  2 筆 high-stakes agreement 因 Android UI、語意範圍、資安或緊急應變文字交由
  maintainer 確認。Maintainer 於 2026-07-31 確認完整 100 筆 synthesis；private
  expected 已寫入 1,800 筆並維持 `annotating`，SHA-256 為
  `69e7c691adec3a1f9f0d7a5060481f460632bb59dd31b48460c9d9c5c318284f`。Public
  annotation progress 為 1,800/1,960，SHA-256 為
  `ffa89ced0b58374fe71d6f1e7b25327e62a95e43cb5a9f1e688d21f9368073ad`；全程未執行
  zhtw 或競品 converter。
- [x] 2026-07-31 完成正式 expected annotation batch 019：從 frozen inputs 的
  固定 offset 1,800 取 100 筆。Codex first pass 的
  accepted output 由 10 個 input-only 群組（每組 10 筆）合併，4 個無輸出任務已
  ...
  Maintainer 於 2026-07-31 確認完整 100 筆 synthesis；private expected 已寫入
  1,900 筆並維持 `annotating`，SHA-256 為
  `c766187b8da7e10581596716b9e32052538673a0d95e991451d08f9c0ba04594`。Public
...
- [x] 2026-07-31 完成最終 expected annotation batch 020：從 frozen inputs 的
  ...
  16 筆不同，另有 24 筆僅 acceptable variants 不同；Codex 以 1 個 input-only
