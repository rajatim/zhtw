...
2. 提交者在其可處分權利範圍內，對提交文字適用
   ...
   複製、修改、散佈、翻譯與公開使用。CC0 的正式法律文字優先於本說明。
...
- 自動檢查只降低風險，不能保證偵測所有個資或祕密。
- 偵測到敏感內容時整筆拒絕，要求提交者提供已自行去識別的新句子；工具不得靜默
  遮罩或改寫原句。
- Issue 中的情境說明不會匯入 benchmark，也不會直接決定 domain、risk 或 expected。
- 每筆接受的 input 會保存公開 issue URL、issue 編號、投稿者 GitHub login、提交時間與
  maintainer 審核當下的 issue body SHA-256，供日後稽核同意來源。
