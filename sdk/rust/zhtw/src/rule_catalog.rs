...
pub(crate) fn legacy_custom_rule_id(source: &str, target: &str) -> String {
    let canonical = format!(
        "{{\"rule_class\":\"custom\",\"source\":{},\"source_locale\":\"cn\",\"target\":{}}}",
        quote(source),
        quote(target)
    );
    let digest = sha256(canonical.as_bytes());
    let hex = to_hex(&digest);
    format!("legacy:cn:custom:{}", &hex[..24])
}
