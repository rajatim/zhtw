...
REPORT = ROOT / "docs/reports/blind-v2-candidate-promotion-batches-001-050-2026-07-29.md"
...
def test_committed_candidates_are_reproducible_input_only_and_deduplicated() -> None:
    ...
    generated, report = build_pool(
        list(DECISIONS),
        output=POOL,
        created_at="2026-07-29T12:43:06+08:00",
    )
    ...
    assert committed["stats"] == {
        ...
        "by_risk": {
            "baseline_guard": 1244,
            "candidate_gap": 1778,
            "over_conversion_guard": 1328,
        },
        "by_source_class": {
            "permissive_license": 1469,
            "project_original": 1455,
            "public_domain": 1426,
        },
        "by_source": {
    ...
    assert report["confirmed_eligible"] == 4357
    assert report["promoted"] == 4350
