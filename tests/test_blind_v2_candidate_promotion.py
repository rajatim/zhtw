...
REPORT = ROOT / "docs/reports/blind-v2-candidate-promotion-batches-001-059-2026-07-30.md"
...
def test_committed_candidates_are_reproducible_input_only_and_deduplicated() -> None:
    ...
    generated, report = build_pool(
        list(DECISIONS),
        output=POOL,
        created_at="2026-07-30T02:59:45+08:00",
    )
    ...
    assert committed["stats"] == {
        ...
        "by_risk": {
            "baseline_guard": 1496,
            "candidate_gap": 2152,
            "over_conversion_guard": 1504,
        },
        "by_source_class": {
            "permissive_license": 1743,
            "project_original": 1742,
            "public_domain": 1667,
        },
        "by_source": {
    ...
    assert report["confirmed_eligible"] == 5162
    assert report["promoted"] == 5152
